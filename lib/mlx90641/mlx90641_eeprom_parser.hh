...
///
/// Scale exponents are read from the EEPROM, so a corrupt image can decode one
/// far past this bound (the six-bit alpha-scale field is biased by 20, reaching
/// 83). Shifting that far is undefined behaviour, so the helpers saturate instead.
...
public:
    ...
    /// The scan stops once every slot is filled: max_broken_pixels of them are
    /// correctable, and the last one only has to show that there are more.
